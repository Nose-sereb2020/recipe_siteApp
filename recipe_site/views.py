...
def request(request):
    ...
    r = requests.get('https://app.rakuten.co.jp/services/api/Recipe/CategoryRanking/20170426?applicationId={}&elements=recipeTitle,foodImageUrl,rank'.format(api_key), headers=headers_dic)
    data = r.json()
    print(data)
    try:
        data = json.dumps(data['result'], ensure_ascii=False)
    except:
        return render(request, 'recipe_site/maintenance.html')
